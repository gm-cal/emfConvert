...
namespace Utils.DataType.DIB.Offset{
    public class BITMAPV5HEADER{
        public const int   HeaderSize     = 124;          // ヘッダーのサイズ
        ...
        public uint  Intent           { get; private set; } = 0;               // 描画意図
        public uint  ProfileData      { get; private set; } = 0;               // プロファイルデータのオフセット
        public uint  ProfileSize      { get; private set; } = 0;               // プロファイルデータのサイズ
        public uint  Reserved         { get; private set; } = 0;               // 予約

        public BITMAPV5HEADER(int width, int height){
            Width = width;
            Height = height;
        }
        ...
        public byte[] ToBytes(){
            ...
            BitConverter.GetBytes(Intent)       .CopyTo(header, offset); offset += 4;
            BitConverter.GetBytes(ProfileData)  .CopyTo(header, offset); offset += 4;
            BitConverter.GetBytes(ProfileSize)  .CopyTo(header, offset); offset += 4;
            BitConverter.GetBytes(Reserved)     .CopyTo(header, offset); offset += 4;
            return header;
        }
    }
}